...
public class FlleReaderTestCase1 {
	...
	public static Function<String, Order> mapToOrder = (line) -> {
		...
		if(p.length <5 || p.length > 5){
			return new Order(Integer.parseInt(p[0]),null,null,null,0,Arrays.deepToString(p));
	    }
		return new Order(Integer.parseInt(p[0]), p[1], p[2], p[3], Float.parseFloat(p[4]),null);
	};
