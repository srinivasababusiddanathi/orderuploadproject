...
public class OrderServiceImpl implements OrderService {
	...
	private Function<String, Order> mapToOrder = (line) -> {
		...
		if (vales.length < 5 || vales.length > 5) {
			return new Order(Integer.parseInt(vales[0]), null, null, null, 0, Arrays.deepToString(vales));
		}
		return new Order(Integer.parseInt(vales[0]), vales[1], vales[2], vales[3], Float.parseFloat(vales[4]), null);
	};
